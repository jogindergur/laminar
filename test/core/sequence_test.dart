...
void main() {
  group('Sequence Widget', () {
    const config = VideoConfig(id: 'test_seq', width: 1920, height: 1080, fps: 30, durationInFrames: 100);

    Widget buildSequenceEnv(int currentFrame, Sequence sequence) {
      return Directionality(
        textDirection: TextDirection.ltr,
        child: CompositionProvider(config: config, frame: currentFrame, child: sequence),
      );
    }

    testWidgets('child is not rendered before "from" frame', (tester) async {
      await tester.pumpWidget(buildSequenceEnv(5, Sequence(from: 10, child: Container(key: const Key('child')))));

      expect(find.byKey(const Key('child')), findsNothing);
    });

    testWidgets('child is rendered at "from" frame with shifted currentFrame=0', (tester) async {
      int? childFrame;
    ...
    testWidgets('child is rendered correctly during duration with correct relative frame', (tester) async {
      int? childFrame;
      await tester.pumpWidget(
        buildSequenceEnv(
          15,
    ...
    testWidgets('layout=true still mounts child outside active window but invisible', (tester) async {
      ...
      await tester.pumpWidget(
        buildSequenceEnv(5, Sequence(from: 10, layout: true, child: Container(key: const Key('child')))),
      );
