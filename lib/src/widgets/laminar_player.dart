...
import '../core/composition.dart';
import '../core/laminar_controller.dart';
import '../export/export_mp4.dart';
...
///
/// Wraps a [Composition] widget and provides transport controls,
/// scrubbing, and an MP4 exporting UI.
class LaminarPlayer extends StatefulWidget {
  final String title;
  final IconData? icon;
  final Color accent;
  ...
  /// Optional callback triggered when MP4 export completes natively.
  /// Used to integrate with sharing plugins outside the library.
  final Future<void> Function(String outputPath)? onExportSuccess;
  ...
  @override
  State<LaminarPlayer> createState() => _LaminarPlayerState();
}

class _LaminarPlayerState extends State<LaminarPlayer> {
  ...
  void initState() {
    ...
    _logs.add(
      '✓ Composition "${widget.config.id}" loaded — '
      '${widget.config.durationInFrames} frames @ ${widget.config.fps}fps',
    );
    _logs.add('✓ LaminarController created — ready for playback.');
  }
  ...
  Future<void> _exportMp4() async {
    ...
    setState(() {
      _isExporting = true;
      _logs.clear();
      _logs.add('▶ Generating ${widget.config.durationInFrames} frames…');
    });
    ...
    try {
      ...
      for (int i = 0; i < widget.config.durationInFrames; i++) {
        _ctrl.seekTo(i);
        ...
        if (i % 5 == 0 && mounted) {
          setState(() {
            _logs.add('  → Captured & Encoded $i/${widget.config.durationInFrames}');
          });
        }
      }
      ...
      // 4. Offer to share/save the file natively via external callback
      if (!kIsWeb && widget.onExportSuccess != null) {
        await widget.onExportSuccess!(outputPath);
      }
    } catch (error, stack) {
      if (!mounted) return;
      setState(() {
  ...
  Widget build(BuildContext context) {
    final timeSec = _ctrl.frame / widget.config.fps;
        ...
        children: [
                      ...
                      children: [
                              ...
                              child: Container(
                                ...
                                child: AspectRatio(
                                  ...
                                  child: Composition<void>(
                                    ...
                                    defaultProps: null,
                                    serialize: (_) => {},
                                    component: (ctx, _) => widget.component(ctx),
                                    controller: _ctrl,
                                    loop: false,
                                  ),
                                ),
                        ...
                        SliderTheme(
                          ...
                          child: Slider(
                            ...
                            min: 0,
                            max: (widget.config.durationInFrames - 1).toDouble(),
                  ...
                  child: Column(
                    ...
                    children: [
                      Padding(
                        ...
                        child: Row(
                          children: [
                            Icon(Icons.terminal, size: 13, color: widget.accent),
                            const SizedBox(width: 6),
                            const Text(
                              'Render Console',
                              style: TextStyle(
                      ...
                      Expanded(
                        child: _RenderLog(logs: _logs, accent: widget.accent),
                      ),
                    ],
                  ),
                ),
          ...
          Container(
            ...
            child: Wrap(
              ...
              children: [
                _stat('frame', '${_ctrl.frame} / ${widget.config.durationInFrames - 1}'),
                _div(),
                _stat('status', _ctrl.status.name),
                _div(),
                _stat('progress', '${(_ctrl.progress * 100).toStringAsFixed(1)}%'),
                _div(),
                _stat('fps', widget.config.fps.toString()),
                _div(),
                SizedBox(
                  width: 100,
                  height: 4,
